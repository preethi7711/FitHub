"""
Full multi-exercise rep counter engine using MediaPipe + OpenCV.

Install:
    pip install opencv-python mediapipe numpy

Run:
    python full_fitness_rep_counter.py
"""
...
# ----------------------------- Configuration ----------------------------- #
EXERCISE_MODE = "pushup"  # squat | pushup | curl | shoulder_press | lunge
CAMERA_INDEX = 0
...
class ExerciseCounter(ABC):
    ...
    def _apply_standard_rep_logic(self, angle: float,
                                  down_condition: bool,
                                  up_condition: bool) -> None:
        if down_condition:
            self._down_frames += 1
...
class CurlCounter(ExerciseCounter):
    ...
    def update(self, landmarks) -> Tuple[int, str, Optional[float]]:
        ...
        self._apply_standard_rep_logic(
            angle=angle,
            down_condition=angle <= 60.0,   # arm flexed
            up_condition=angle >= 145.0,     # arm extended
        )
        return self.reps, self.stage, angle
...
class ShoulderPressCounter(ExerciseCounter):
    ...
    def update(self, landmarks) -> Tuple[int, str, Optional[float]]:
        ...
        # Use larger elbow angle for press lockout emphasis.
...
class LungeCounter(ExerciseCounter):
    ...
    def update(self, landmarks) -> Tuple[int, str, Optional[float]]:
        ...
        split_dist = abs(l_ankle[0] - r_ankle[0])  # normalized horizontal split
        angle = self._smooth(float(np.min(vals)))
        if angle is None:
            return self.reps, self.stage, None

        down_ok = split_dist >= 0.18 and angle <= 115.0
        up_ok = angle >= 155.0

        self._apply_standard_rep_logic(
            angle=angle,
            down_condition=down_ok,
            up_condition=up_ok,
        )
        return self.reps, self.stage, angle


def create_counter(mode: str) -> ExerciseCounter:
    mode = mode.lower()
    if mode == "squat":
        return SquatCounter()
    if mode == "pushup":
    ...
    if mode == "lunge":
        return LungeCounter()
    raise ValueError(
        "Invalid EXERCISE_MODE. Use one of: squat, pushup, curl, shoulder_press, lunge"
    )


def draw_overlay(frame, exercise_name: str, reps: int, stage: str,
                 angle: Optional[float], fps: float) -> None:
    ...
    cv2.putText(frame, f"FPS: {fps:.1f}", (260, 141),
...
def main() -> None:
    counter = create_counter(EXERCISE_MODE)
    ...
    with mp_pose.Pose(
        ...
        while True:
            ...
            reps, stage, angle = counter.update(landmarks)

            now = time.time()
            fps = 1.0 / max(now - prev_time, 1e-8)
            prev_time = now

            draw_overlay(frame, counter.name, reps, stage, angle, fps)
            cv2.imshow("Full Fitness Rep Counter", frame)
