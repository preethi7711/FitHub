"""
Interactive fitness rep counter with Bird Flip mini-game.

Install:
    pip install opencv-python mediapipe numpy

Run:
    python bird_flip_fitness_game.py
"""
...
def draw_hud(frame, mode: str, reps: int, stage: str, angle: Optional[float], fps: float) -> None:
    ...
    if GAME_MODE:
        cv2.putText(frame, "Bird Flip: ON", (x0 + 12, y0 + 238),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 2)
    else:
        cv2.putText(frame, "Bird Flip: OFF", (x0 + 12, y0 + 238),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (180, 180, 180), 2)

    cv2.putText(frame, "Press 'r' to restart game | 'q' to quit",
                (10, frame.shape[0] - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 1)
...
def main() -> None:
    ...
    with mp_pose.Pose(
        ...
        while True:
            ...
            cv2.imshow("Bird Flip Fitness Game", out)
