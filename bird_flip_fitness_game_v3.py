...
class Bird:
    ...
    def update(self, assist_mode: bool) -> None:
        if self.game_over:
            return
        ...
        effective_gravity = self.gravity * 0.3 if assist_mode else self.gravity
        self.velocity += effective_gravity
        if assist_mode:
            self.velocity = min(self.velocity, 3.0)
    ...
    def draw(self, canvas, score: int, exercise_label: str, assist_mode: bool) -> None:
        canvas[:] = (25, 30, 45)
        ...
        if assist_mode and self.game_started and self.physics_active() and not self.game_over:
            cv2.putText(canvas, "ASSIST MODE (do a rep!)", (self.width // 2 - 190, 80),
...
def main() -> None:
    ...
    with mp_pose.Pose(
        ...
        while True:
            ...
            if GAME_MODE:
                assist_mode = (time.time() - last_rep_time) > ASSIST_TRIGGER_SECONDS
                bird.update(assist_mode=assist_mode)
                out = np.zeros((bird.height, bird.width, 3), dtype=np.uint8)
                bird.draw(
                    out,
                    score=total_score,
                    exercise_label=_mode_label(current_mode),
